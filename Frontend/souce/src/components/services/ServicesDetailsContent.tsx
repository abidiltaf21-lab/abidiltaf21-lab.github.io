...
const ServicesDetailsContent = () => {
    ...
    useEffect(() => {
        const fetchService = async () => {
            try {
                setLoading(true);
                const { data } = await apiService.getService(id || "1");
                
                if (data) {
                    let groups = [];
                    try {
                        groups = JSON.parse(data.groupsJson || data.GroupsJson || '[]');
                    } catch (e) {
                        groups = [];
                    }
                    ...
                    let process = [];
                    try {
                        process = JSON.parse(data.processJson || data.ProcessJson || '[]');
                    } catch (e) {
                        process = [];
                    }
                    ...
                    setService({
                        ...
                        mainTitle: data.mainTitle || data.MainTitle || data.title || data.Title,
                        description: data.description || data.Description || data.text || data.Text,
                        secondaryDescription: data.secondaryDescription || data.SecondaryDescription || '',
        ...
        const applyFallback = () => {
            const localItem = ServicesDetailsData.find(s => s.id === parseInt(id || "1")) || (ServicesDetailsData[0] as any);
                                ...
                                <div className="process-box p-40 rounded-20 bg-gray">
                                    ...
                                    <div className="process-style-one">
                                        {service.process.map((step: any, index: number) => (
                                            <div className="process-style-one-item sp-card" key={index}>
                                                <span>0{index + 1}</span>
                                                <h4>{step.title}</h4>
                                                <p>{step.text}</p>
                                            </div>
