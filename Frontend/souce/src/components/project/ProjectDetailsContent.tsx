...
const ProjectDetailsContent = ({ projectData }: { projectData: DataType }) => {
    const { thumbFull, title, id } = projectData;
    let { videoUrl } = projectData;

    // Support both nested (old) and flat (new .NET) structures
    const projectType = projectData.projectType || "Animation";
    const industry = projectData.industry || "Creative";
    const challenge = projectData.challenge || "Creating a compelling narrative for the brand.";
    const solution = projectData.solution || "Implementation of high-fidelity 2D/3D animation sequences.";
    const result = projectData.result || "Increased brand engagement and user retention.";
    const skills = projectData.skills || "After Effects, Cinema 4D";
    const duration = projectData.duration || "4 Weeks";
    const testimonial = projectData.testimonial || "نه دی ذکر شوی";

    // Normalize Cloudinary .ts to .mp4
    if (videoUrl && videoUrl.includes('cloudinary.com') && videoUrl.toLowerCase().endsWith('.ts')) {
    ...
    useEffect(() => {
        // Increment views on load, ensuring it only happens once per mount
        if (!viewIncremented.current && id) {
            // incrementProjectViews(id); // Temporarily removed
                        ...
                        >
                            {videoUrl ? (
                                <div className="project-video-container" style={{ position: 'relative', width: '100%', paddingTop: '56.25%', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.3)', background: '#000' }}>
                                    {(videoUrl.match(/\.(mp4|webm|mov|ts)(\?.*)?$/i) || (videoUrl.includes('cloudinary.com'))) ? (
                                        <video
                                            src={videoUrl}
                                            autoPlay
                                            muted
                                ...
                                <img src={thumbFull?.startsWith('http') ? thumbFull : `/assets/img/projects/${thumbFull || '1-full.jpg'}`} alt={title} />
                            )}
                        </div>
                    </div>
                </div>
                        ...
                        <div className="single-grid">
                            <div className="item-grid-colum">
                                ...
                                <div className="right-info">
                                    ...
                                    {testimonial && testimonial !== "نه دی ذکر شوی" && (
                                        <div className="client-testimonial-box mt-40 p-30 rounded-20" style={{ background: 'var(--color-primary)', color: '#fff' }}>
                                            ...
                                            <h4 style={{ color: '#fff', fontStyle: 'italic' }}>"{testimonial}"</h4>
